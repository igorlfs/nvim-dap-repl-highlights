...
function M.get_parser(bufnr, lang, opts)
    ...
    if not valid_lang(lang) then
        if not parsers[bufnr] then
            local err_msg = string.format("Parser not found for buffer %s: language could not be determined", bufnr)
        ...
        if not parser then
            local err_msg = string.format('Parser could not be created for buffer %s and language "%s"', bufnr, lang)
            if should_error then
                error(err_msg)
            end
            return nil, err_msg
